...
namespace NMISRTOCXI.Controllers
{
	public class AntemortemController : Controller
	{
        ...
        {
            var existingAntemortem = await _unitOfWork.Antemortems.Get(c => c.Id == anteRowId && c.ReceivingReportId == Id);
			...
			var antemortem = await _unitOfWork.Antemortems
        ...
        public async Task<IActionResult> Delete(Guid Id, int anteRowId)
        {
            ...
            var entityToDelete = await _unitOfWork.Antemortems.Get(c => c.Id == anteRowId && c.ReceivingReportId == Id);
		...
		public async Task<IActionResult> Create(Guid Id, int conductHead, double conductWeight, int issue, int cause, string remarks, string others)
		{
            ...
            try
            {
                ...
                _unitOfWork.Antemortems.Add(result);
                await _unitOfWork.Save();
		...
		public async Task<IActionResult> Index(Guid myVariable)
		{
			...
			var response = await _unitOfWork.Antemortems.GetAll(c => c.ReceivingReportId == myVariable, includeProperties: "ReceivingReport");
		...
		public async Task<IActionResult> Details(int? Id)
		{
			...
			var antemortem = await _unitOfWork.Antemortems.Get(c => c.Id == Id, includeProperties: "ReceivingReport");
			if (antemortem == null)
			{
				return NotFound();
			}
		...
		//	{
		//		_unitOfWork.Antemortems.Add(response);
		//		await _unitOfWork.Save();
		...
		//	var antemortem = await _unitOfWork.Antemortems.Get(c => c.Id == Id);
		//	if (antemortem == null)
		//	{
		//		return NotFound();
		//	}
		...
		public async Task<IActionResult> Edit(int Id, Antemortem request)
		{
			...
			if (ModelState.IsValid)
			{
                _unitOfWork.Antemortems.Update(request);
                await _unitOfWork.Save();
		...
		//	if (antemortem != null)
		//	{
		//		_unitOfWork.Antemortems.Remove(antemortem);
		//	}
